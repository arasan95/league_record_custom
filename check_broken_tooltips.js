...
async function run() {
    ...
    for (let i = 0; i < champs.length; i++) {
        ...
        try {
            ...
            for (const spell of ddSpells) {
                ...
                const failMatches = [];
                desc = desc.replace(/\{\{\s*(.*?)\s*\}\}/gi, (match, p1, offset, fullStr) => {
                    ...
                    if (varRefs) {
                        ...
                        for (const vName of varRefs) {
                             ...
                             if (expr.includes('wslowpercentage') && spell.id === 'AatroxW') {
                                 console.log(`[MATH DEBUG] Aatrox w: expr='${expr}', var='${vName}', valArr=`, valArr);
                             }
                        ...
                        if (!missing) {
                             if (expr.includes('wslowpercentage') && spell.id === 'AatroxW') {
                                  console.log(`[PRE-EVAL] About to call evalMath with vMap:`, vMap);
                             }
                             const r = evalMath(expr, vMap);
                             if (r !== null) return String(r);
                             if (expr.includes('wslowpercentage') && spell.id === 'AatroxW') {
                                  console.log(`[POST-EVAL] evalMath returned null! r=`, r);
                             }
                        }
                    }
                    if (expr.includes('wslowpercentage') && spell.id === 'AatroxW') {
                        console.log(`[FAIL MATCHES] Pushing expr to failMatches:`, expr);
                    }
    ...
    fs.writeFileSync("broken_tooltips.json", JSON.stringify(brokenList, null, 2));
    console.log(`Scan complete! Found ${brokenList.length} broken tooltip nodes. Results saved to broken_tooltips.json.`);
}
